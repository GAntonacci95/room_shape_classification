...
# TODO(rev_voices_fdrscc_leaf_trial): Markdown description  that will appear on the catalog page.
_DESCRIPTION = """
...
# TODO(rev_voices_fdrscc_leaf_trial): BibTeX citation
_CITATION = """
"""


class RevVoicesFdrsccLeafTrial(tfds.core.GeneratorBasedBuilder):
  """DatasetBuilder for rev_voices_fdrscc_leaf_trial dataset."""
  ...
  def _info(self) -> tfds.core.DatasetInfo:
    ...
    # TODO(rev_voices_fdrscc_leaf_trial): Specifies the tfds.core.DatasetInfo object
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    # TODO(rev_voices_fdrscc_leaf_trial): Downloads the data and defines the splits
    # caricare il dataset pkl, fare split ed un po'di pre-processing
    # HATEFUL ABS PATH
    df = p.read_pickle("/nas/home/gantonacci/Thesis/Project/pythonProject/datasets/nn/final_datasets/main_vc_frmp0_fdrCC_for_leaf.pkl")
    dftrain, dfval, dftest = split_tvt(df)

    # TODO(rev_voices_fdrscc_leaf_trial): Returns the Dict[split names, Iterator[Key, Example]]
    return {
  ...
  def _generate_examples(self, split: p.DataFrame):
    """Yields examples."""
    # TODO(rev_voices_fdrscc_leaf_trial): Yields (key, example) tuples from the dataset
