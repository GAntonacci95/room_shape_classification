...
from eg.features.pre_processing import eg_fdrs
...
def run():
    ...
    # eg_fdrs.computeCC(egvoiceacqus_output_dir,
    #                   {"f_s": f_s, "len_min_samps": 1600, "len_max_samps": 5000})
    # warnings.filterwarnings("default")
    egvoicefdrs_output_dir = eg_fdrs.loadCC(egvoiceacqus_output_dir,
                                            {"f_s": f_s, "len_min_samps": 1600, "len_max_samps": 5000})
    # eg_reports_fdrs.create_report(egvoicefdrs_output_dir, "fdrsCC")
    # </editor-fold>

    # creazione dataset "finale":
    # nonostante la previsione, ci mette meno
    # ricomporre X=(60K, ~3.3K), class_label=(60K, 1), volume_m3=(60K, 1) è fattibile ed
    # è ora caricabile in memoria senza eccessivi problemi (1.5GB ca)
    # in modo da poter sfruttare appieno la GPU evitando problematiche di InputPipeline

    # # VOLUME REGRESSION OVER R/L/RL
    dsname = "main_vc_frmp0_fdrCC_for_leaf"
    # eg_finalize_acqus_ds.compute(egvoicefdrs_output_dir, dsname)

    # OK QUANTO SEGUE SERVE A DIMOSTRARE CHE
    # NEL DS LE CLASSI NON SONO SBILANCIATE A CAUSA DELL'USO DELLE FDR :)
    # df = eg_finalize_acqus_ds.load(dsname)
    ...
    # dsu.subsets_handler(dsu.show_field_distro, [dftrain, dfval, dftest], ["class_id"])
    # </editor-fold>
