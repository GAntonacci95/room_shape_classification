...
def acquisitions(df: p.DataFrame, params: dict):
    print("Acquisitions generation")
    tt = ThreadTimer()
    # [s_i[dir_list]]
    ...
    for t in df.itertuples():
        ...
        for sig in sigs:
            ...
            if fs != fss[-1] or fs != rir_df.f_s.values[0]:
                raise Exception("Acqus: incoherent samp rates")

        # SETUP & COMPUTE_RIR
        praroom: Room = None
        try:
            praroom = ru.praroom_from_setup_df(setup_df,
                                               rir_df.material_arg.values[0],
                                               rir_df.f_s.values[0],
                                               rir_df.max_ord.values[0],
                                               rir_df.use_ray.values[0])
            ...
            print("Acqus: Debug break on me! WTH went wrong?")
        ...
        # if np.any(mic_signal == 0) == True or \
        #         np.any(np.isinf(mic_signal)) == True or \
        #         np.any(np.isnan(mic_signal)) == True:
        #     print("debug here...")      # alcuni 0 ci sono...
...
def compute(egsigs_output_dir: str, params: dict) -> None:
    one_to_one_or_many.compute_parallel(
        egsigs_output_dir,
        {
            "handler": acquisitions,
            "params": params
        },
...
def load(egsigs_output_dir: str, params: dict) -> str:
