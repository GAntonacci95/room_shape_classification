...
def time_domain(df: p.DataFrame, params: dict) -> None:
    f_s = params["f_s"]
    win_len = params["win_len"]
    hop_size = params["hop_size"]
    print("Time-dom signal generation")
    tt = ThreadTimer()
    for t in df.itertuples():
        ...
        datum = {
            ...
            "output": nnpostprocutils.log_E(signal.reshape((1, -1)), win_len, hop_size)
        }
...
def compute(anypreproc_output_dir: str, params: dict) -> None:
    one_to_one_or_many.compute_parallel(
        ...
        {"handler": time_domain, "params": params},
