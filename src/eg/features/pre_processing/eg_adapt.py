...
def adapt(df: p.DataFrame, params: dict) -> None:
    if "len_max" in params.keys():
        l0, l = 0, params["len_max"]
    else:
        l0, l = params["l_start"], params["l_end"]
    print("Adaptations generation")
    tt = ThreadTimer()
    for t in df.itertuples():
        ...
        adapted = adapted + (10E-6 * np.random.randn(*adapted.shape))
